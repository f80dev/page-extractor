  ...
  template: `
    <div class="popup-container">
      ...
      <section class="page-info">
        ...
        <span class="url-value" [title]="currentUrl()">{{ truncatedUrl() }}</span>
      </section>
      ...
      @if (isLoading()) {
        ...
        <div class="action-buttons">
          ...
          <button mat-icon-button [matMenuTriggerFor]="moreMenu" title="Plus d'options">
        ...
        <section class="exploration-section">
          ...
          <div class="progress-info">
            ...
            @if (explorationProgress().currentUrl) {
              <span class="current-url" [title]="explorationProgress().currentUrl">
...
export class PopupComponent implements OnInit {
  ...
  private listenForUpdates() {
    chrome.runtime.onMessage.addListener((message: { type: string; payload?: unknown }) => {
      if (message.type === 'POPUP_UPDATE' && message.payload) {
        this.explorationProgress.set(message.payload as Parameters<typeof this.explorationProgress.set>[0]);
      }
    });
  }
  ...
  async extractAndSend() {
    ...
    try {
      ...
      this.snackBar.open(`Erreur : ${msg}`, 'OK', { duration: 5000 });
    } finally {
  ...
  async exploreWithDepth() {
    ...
    try {
      ...
      const msg = error instanceof Error ? error.message : String(error);
      this.snackBar.open(`Erreur : ${msg}`, 'OK', { duration: 3000 });
    }
  }
